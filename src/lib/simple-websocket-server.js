...
class SimpleOnlineStatusServer {
  ...
  broadcastUserStatus(userId, isOnline) {
    ...
    console.log(
      `📢 Broadcasting: User ${userId} is ${isOnline ? "ONLINE" : "OFFLINE"}`,
    );
      ...
      if (clientUserId !== userId) {
        userClients.forEach((client) => {
          if (client.readyState === 1) {
            ...
            console.log(
              `📤 Sent to user ${clientUserId}: User ${userId} is ${isOnline ? "ONLINE" : "OFFLINE"}`,
            );
          }
        });
      }
...
// Start the server
const onlineStatusServer = new SimpleOnlineStatusServer();
...
module.exports = SimpleOnlineStatusServer;
